...
from infer_p3m_portrait_matting.p3m.core.util import get_masked_local_from_global
from infer_p3m_portrait_matting.p3m.core.config import PRETRAINED_VITAE_NORC_MAXPOOLING_BIAS_BASIC_STAGE4_14
...
@register_model
# adopt performer for tokens to token
def ViTAE_noRC_MaxPooling_bias_basic_stages4_14(pretrained=True, **kwargs):
    ...
    if pretrained:
        ckpt = torch.load(PRETRAINED_VITAE_NORC_MAXPOOLING_BIAS_BASIC_STAGE4_14)[
            'state_dict_ema']
...
class ViTAE_noRC_MaxPooling_DecoderV1(nn.Module):
    def __init__(self):
        ...
        self.decoder_final_g = nn.Conv2d(
            out_chan, 3, kernel_size=3, padding=1)  # 32 -> 3
    ...
    def forward(self, x, indices, feas):
        ...
        d4_l = F.max_unpool2d(
            d4_l, indices[-1], kernel_size=2, stride=2)  # 256
        d3_l = self.tfi_3(d4_g, d4_l, feas[-1])  # 256, 256, 256 -> 256
        d3_l = self.decoder3_l(d3_l)  # 256 -> 128
        d3_l = F.max_unpool2d(
            d3_l, indices[-2], kernel_size=2, stride=2)  # 128
